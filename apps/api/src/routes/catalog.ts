...
export function registerCatalogRoutes(app: Hono<Env>): void {
  ...
  app.get('/api/v1/businesses/:businessId/products', async (c) => {
    ...
    const result = await c.env.DB.prepare(
      ...
      .bind(includeArchived ? 1 : 0, membership.businessId, includeArchived ? 1 : 0, query, search, search, search, search, search)
      .all();
  ...
  app.get('/api/v1/businesses/:businessId/catalog-options', async (c) => {
    ...
    const [categories, brands] = await Promise.all([
      c.env.DB
        .prepare("SELECT id,name FROM categories WHERE business_id=? AND status='active' ORDER BY name LIMIT 200")
        .bind(membership.businessId)
        .all(),
      c.env.DB
        .prepare("SELECT id,name FROM brands WHERE business_id=? AND status='active' ORDER BY name LIMIT 200")
        .bind(membership.businessId)
        .all(),
    ]);
  ...
  app.post('/api/v1/businesses/:businessId/catalog-options', async (c) => {
    ...
    if ((type !== 'category' && type !== 'brand') || !name || name.length > 120)
      return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Jenis dan nama katalog wajib valid' } }, 422);
  ...
  app.post('/api/v1/businesses/:businessId/products/:productId/variants', async (c) => {
    ...
    const product = await c.env.DB.prepare("SELECT id FROM products WHERE business_id=? AND id=? AND status='active'")
      .bind(membership.businessId, productId).first();
    if (!product) return c.json({ error: { code: 'NOT_FOUND', message: 'Not found' } }, 404);
    const body = await c.req.json<Partial<{
      label: string; sku: string; barcode: string; price_minor: string | number;
      cost_minor: string | number; options_json: string;
    }>>();
    ...
    if (!label || !sku || label.length > 120 || sku.length > 80 ||
    ...
    const now = new Date().toISOString();
    try {
      const outlets = await c.env.DB.prepare("SELECT id FROM outlets WHERE business_id=? AND status='active'")
        .bind(membership.businessId).all<{ id: string }>();
      const statements = [
        c.env.DB.prepare(
        ...
        ).bind(id, membership.businessId, productId, label, sku, body.barcode?.trim() || null, price ?? null, cost ?? null, body.options_json?.trim() || '{}', now, now),
      ];
  ...
  app.patch('/api/v1/businesses/:businessId/products/:productId/variants/:variantId', async (c) => {
    ...
    ).bind(membership.businessId, c.req.param('productId'), variantId).first();
    if (!current) return c.json({ error: { code: 'NOT_FOUND', message: 'Not found' } }, 404);
    const body = await c.req.json<Partial<{ status: 'active' | 'archived'; label: string; barcode: string; price_minor: string | number; cost_minor: string | number }>>();
    ...
    if (body.label !== undefined) { const value = body.label.trim(); if (!value || value.length > 120) return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid label' } }, 422); fields.push('label=?'); values.push(value); }
    if (body.barcode !== undefined) { fields.push('barcode=?'); values.push(body.barcode.trim() || null); }
    for (const key of ['price_minor', 'cost_minor'] as const) {
      if (body[key] === undefined) continue;
      const value = integer(body[key]);
      if (value === undefined || value < 0) return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid variant amount' } }, 422);
      fields.push(`${key}=?`); values.push(value);
    }
    if (body.status) { fields.push('status=?'); values.push(body.status); }
    if (!fields.length) return c.json({ error: { code: 'VALIDATION_ERROR', message: 'No changes supplied' } }, 422);
    fields.push('updated_at=?'); values.push(new Date().toISOString(), membership.businessId, c.req.param('productId'), variantId);
    try {
      await c.env.DB.prepare(`UPDATE product_variants SET ${fields.join(',')} WHERE business_id=? AND product_id=? AND id=?`).bind(...values).run();
    } catch {
  ...
  app.post('/api/v1/businesses/:businessId/products', async (c) => {
    ...
    if (body.category_id || body.brand_id) {
      const metadata = await c.env.DB.prepare(
        ...
        .bind(body.category_id ?? '', membership.businessId, body.brand_id ?? '', membership.businessId)
        .first<{ category_id: string | null; brand_id: string | null }>();
      if ((body.category_id && metadata?.category_id !== body.category_id) || (body.brand_id && metadata?.brand_id !== body.brand_id))
  ...
  app.patch('/api/v1/businesses/:businessId/products/:productId', async (c) => {
    ...
    if (!c.env.DB) return unavailable(c);
    const body = await c.req.json<Partial<{
      name: string; barcode: string; description: string; unit_key: string;
      price_minor: string | number; cost_minor: string | number; tax_rate_bp: string | number;
      reorder_level: string | number; category_id: string | null; brand_id: string | null;
    ...
    if (body.name !== undefined) {
      ...
      if (!value || value.length > 160) return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid name' } }, 422);
      fields.push('name=?'); values.push(value);
    }
    if (body.barcode !== undefined) { fields.push('barcode=?'); values.push(body.barcode.trim() || null); }
    if (body.description !== undefined) { fields.push('description=?'); values.push(body.description.trim() || null); }
    if (body.unit_key !== undefined) { fields.push('unit_key=?'); values.push(body.unit_key.trim() || 'pcs'); }
    for (const [key, label] of [['price_minor', 'price'], ['cost_minor', 'cost'], ['tax_rate_bp', 'tax'], ['reorder_level', 'reorder']] as const) {
      if (body[key] === undefined) continue;
      const value = integer(body[key]);
      if (value === undefined || value < 0 || (label === 'tax' && value > 10000))
        return c.json({ error: { code: 'VALIDATION_ERROR', message: 'Invalid numeric field' } }, 422);
      fields.push(`${key}=?`); values.push(value);
    }
    if (body.category_id !== undefined) { fields.push('category_id=?'); values.push(body.category_id || null); }
    if (body.brand_id !== undefined) { fields.push('brand_id=?'); values.push(body.brand_id || null); }
    if (body.status) { fields.push('status=?'); values.push(body.status); }
    if (body.category_id || body.brand_id) {
      const metadata = await c.env.DB.prepare(
        `SELECT (SELECT id FROM categories WHERE id=? AND business_id=? AND status='active') AS category_id,
  ...
  app.get('/api/v1/businesses/:businessId/inventory/movements', async (c) => {
    ...
    if (
      outletId &&
      !(await canAccessOutlet(c.env.DB, membership.businessId, membership.memberId, membership.allOutlets, outletId))
    )
      return c.json({ error: { code: 'NOT_FOUND', message: 'Not found' } }, 404);
    const scope = membership.allOutlets
  ...
  app.post('/api/v1/businesses/:businessId/inventory/adjustments', async (c) => {
      ...
      return c.json(
        {
          error: {
            ...
            message: 'Outlet, variant, quantity, cost, reason and a valid idempotency key are required',
          },
        },
        422,
      );
    ...
    try {
      await c.env.DB.batch([
        ...
        c.env.DB.prepare(
          ...
          createId(), membership.businessId, body.outlet_id, body.variant_id, 'adjustment', delta,
          cost, 'adjustment', sourceId, membership.memberId, idempotencyKey ?? null, now,
        ),
      ]);
    } catch (error) {
    ...
    return adjustmentResult(c.env.DB, membership.businessId, body.outlet_id, body.variant_id, sourceId, false, 201);
  });
}

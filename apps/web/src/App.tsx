...
function ReceiptDialog({ receipt, onClose, onRefund, onNotice }: { receipt: ReceiptData; onClose: () => void; onRefund: (receipt: ReceiptData) => void; onNotice: (notice: { tone: 'success' | 'error'; message: string }) => void }) {
  ...
  async function refund() {
    if (!reason.trim()) { onNotice({ tone: 'error', message: 'Alasan refund wajib diisi.' }); return; }
    setRefundLoading(true);
    try {
      await api('/api/v1/refunds', { method: 'POST', body: JSON.stringify({ saleId: receipt.id, reason: reason.trim() }) });
      onRefund({ ...receipt, status: 'refunded' });
      onNotice({ tone: 'success', message: 'Transaksi berhasil direfund dan stok dikembalikan.' });
    } catch (error) { onNotice({ tone: 'error', message: error instanceof Error ? error.message : 'Refund gagal.' }); }
...
function Inventory({ products }: { products: Product[] }) { return <section className="inventory-view"><div className="section-intro"><div><p className="eyebrow">MASTER DATA</p><h2>Inventory</h2><p>Monitor ketersediaan produk di outlet aktif.</p></div><button className="primary-button" onClick={() => window.alert('Gunakan endpoint POST /api/v1/inventory/adjust untuk penyesuaian stok.') }><Plus size={17} /> Sesuaikan stok</button></div><div className="table-card"><div className="table-toolbar"><label className="search-box compact"><Search size={17} /><input placeholder="Cari inventory..." aria-label="Cari inventory" /></label><span>{products.length} produk aktif</span></div><div className="inventory-table"><div className="table-row table-header"><span>Produk</span><span>SKU</span><span>Stok</span><span>Status</span><span>Harga jual</span></div>{products.map((product) => <div className="table-row" key={product.id}><span><b>{product.name}</b><small>{product.unit}</small></span><span>{product.sku}</span><span>{product.stock}</span><span><em className={product.stock <= product.reorderPoint ? 'status warning' : 'status good'}>{product.stock <= product.reorderPoint ? 'Menipis' : 'Aman'}</em></span><span>{formatIdr.format(product.price)}</span></div>)}</div></div></section>; }
function Customers({ customers }: { customers: Customer[] }) { return <section className="inventory-view"><div className="section-intro"><div><p className="eyebrow">MASTER DATA</p><h2>Pelanggan</h2><p>Profil pelanggan yang tersimpan di business aktif.</p></div><button className="primary-button" onClick={() => window.alert('Gunakan endpoint POST /api/v1/customers untuk menambah pelanggan.') }><Plus size={17} /> Tambah pelanggan</button></div><div className="table-card"><div className="table-toolbar"><span>{customers.length} pelanggan</span><span>Data tenant aktif</span></div><div className="inventory-table"><div className="table-row table-header"><span>Nama</span><span>Telepon</span><span>Email</span><span>Terdaftar</span></div>{customers.length ? customers.map((customer) => <div className="table-row customer-row" key={customer.id}><span><b>{customer.name}</b></span><span>{customer.phone ?? '—'}</span><span>{customer.email ?? '—'}</span><span>{customer.createdAt ? new Date(customer.createdAt).toLocaleDateString('id-ID') : '—'}</span></div>) : <div className="empty-state"><Users size={25} /><span>Belum ada pelanggan.</span><small>Tambahkan pelanggan saat transaksi atau dari master data.</small></div>}</div></div></section>; }
